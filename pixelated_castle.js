...
pattern: "<~ ~ 0 0 0 1 1 1 2 2 ~ ~>/8".pickRestart([
  ...
  n("7 [5 <[6 5 6] 6>] 2 [2 3 4]".add("<[<0,1> <1,0>] [0,2,4]> <[3,5,7]>")).set(chords).voicing().dec(.2).gain("[1 0.5 0.5 0.5]".slow(2)).room("1"),
...
neeow: "<~ ~ ~ ~ ~ ~ ~ 0 1 2 0 ~>/8".pickRestart([
  note("<c2 ~@7>").s("sine").penv(74).pdec(1.5).patt(0).attack(0.01).decay(1.5).sustain(0.1).release(0.3).echo(18,1/6,.8).room(0.9).postgain(0.3),
  note("c7@8 ~@7").s("sine").penv(-74).pdec(1.5).attack(0.01).decay(1.5).sustain(0.1).release(0.3).pan(sine.slow(2)).gain(0.7),
  stack(
...
drums: "<~ ~ 0 1 [0,1,2] [1,2] [1,2] [1,2] [0,2,3] 0 0 ~>/8".pickRestart([
  ...
  "<[bd [bd bd] rim ~] [bd ~ sd ~] [rim rim bd sd]>*2"
]).pickOut({
...
}).when("3".eq("<~ ~ 0 1 [0,1,2] [1,2] [1,2] [1,2] [0,2,3] 0 0 ~>/8"), x => x.room(0.5).delay(0.5))
